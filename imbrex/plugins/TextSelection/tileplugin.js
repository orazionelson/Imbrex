	...
	*/
	...
	$.extend(TextSelector.prototype, {
		...
		addHighlightMarkers: function(JSONobj){
			...
			//console.log(start[0].childNodes.item(JSONobj.StartChild));//.childNodes.lenght);
			//if(start[0].childNodes.item(JSONobj.StartChild)){
			var sNode = start[0].childNodes.item(JSONobj.StartChild);
			//console.log(sNode);
			//console.log(JSONobj.StartOffset);
			range.setStart(sNode, JSONobj.StartOffset);
			//	}
			//else{sNode=['pippo'];}
			//if(end[0].childNodes.item(JSONobj.EndChild)){
			...
			//console.log(typeof(sNode));
			 // create new range from selObj
...
var TS={
	...
	*/
	start:function(mode){
		...
		TILE.engine.insertModeButtons('<span class="menuitem"><ul><li><a id="getHLite" class="btn btn-default btn-sm btnIconLarge getHLite" title="Activate Highlight Mode"><span class="glyphicon glyphicon-pencil" aria-hidden="true"></span></a></li></ul></span>','topleft',mode.name);
	...
	attachButtons:function(sel){
		...
		// find span tag and attach buttons
		if($("span."+id).length>1){
			$("<span id=\"deleteHLite"+id+"\" class=\"button\"><span class=\"glyphicon glyphicon-remove\" title=\"Delete\" aria-hidden=\"true\"></span></span>").appendTo($("span."+id+":eq("+($("span."+id).length-1)+")"));
		} else {
			$("<span id=\"deleteHLite"+id+"\" class=\"button\"><span class=\"glyphicon glyphicon-remove\" title=\"Delete\" aria-hidden=\"true\"></span></span>").appendTo($("span."+id));
		}
