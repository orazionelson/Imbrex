...
var Label=function(args){
	...
	$('body').on("click", "#labelList > .labelItem", function(e){
		...
		//var xx = $("body:first").trigger("labelSelected",[{name:$(this).text(),id:id}]);
		//console.log(xx);
	});
	...
	*/
	start:function(mode){
		...
		//for(var d in data.labels){
			//self.lbls.push({id:data.labels[d].id,type:'labels',jsonName:'labels',obj:data.labels[d]});
			//self.lbls.push({id:data.labels[d].id,type:'labels',jsonName:'labels',obj:data.labels[d]});
		//}
		//var self=preLbls.obj;
		//console.log('vv');
		//console.log(self.lbls);
		/*var self=this;
		...
		var html='<div class="panel panel-warning"><div class="toolbar panel-heading col-xs-12" id="addLabelToolbar"><span class="menuitem col-sm-3 pluginTitle">Labels</span><span class="menuitem col-sm-6"><input id="filterLabelText" class="form-control input-sm" type="text" value="" /></span></div><div id="labelList" class="az panel-body"></div></div>';
		...
		//$("body").on('click','.labelItem',{obj:self},self.activeObjHandle);
		...
		var ach=$('#az_activeBox').height();
		$('#labelList').height(ach-33);
		/*console.log('data');		
		console.log(data);
