...
(function($){
	...
	$.extend(FloatingDiv.prototype, {
		...
		*/
		init: function(myID, labels) {
			...
			var self=this;
			var htmlString;
			var my_page_id = window.location.search.split('=')[1];
			//console.log(window.location.search.split('=')[1])
			var stored_labels=$.ajax({
				url:"edit_imbrex_tile/get_series_labels",
				data: {page_id : my_page_id},
				dataType:'json',
				type:'GET',
				async:false
			}).responseText;
			
			var sl = $.parseJSON(stored_labels);
			//console.log(sl);
  ...
  <option value="audi">Audi</option>
			...
			var select = "<select id=\"formLabel1\" name=\"Label1\" class=\"tagCompletedeleteHLiteanno_V0Fc3znO\">";
			
			var selected='';
			for(var l in sl){
				if(l==0) {selected='selected="selected"';}
				else selected='';
				select +="<option id=\""+sl[l].id+"\" value=\""+sl[l].name+"\" "+selected+">"+sl[l].name+"</option>";
				}
			...
			htmlString = '<fieldset class="label_formFieldSet">' +
						...
						'<!--input id="formLabel1" name="Label1" type="text" class="tagCompletedeleteHLiteanno_V0Fc3znO" /-->' + select +
						' &nbsp; <span class="glyphicon glyphicon-plus" title="Add up to 5 Labels" name="Add up to 5 Labels" id="btnAddLabel"></span>' +
						'<span class="glyphicon glyphicon-minus" style="margin-left: 1px; visibility: hidden;" title="Delete last label" name="Delete last label" id="btnDeleteLabel"></span>' +
						'&nbsp; <span style="" class="addRemove">(Add/Remove Label Fields)</span><br />' +
						'</li>' +
						'</ol>' +
						'<input type="submit" class="submit btn btn-success btn-sm" value="Apply" id="submitFloatingDiv">' +
			...
			$('#btnAddLabel').live('click', function() {
				...
				newElem.find('select:first').attr('id', 'formLabel'+newNum).attr('name', 'Label'+newNum);
		...
		createDialog:function(myID) {
			...
			$("button.ui-dialog-titlebar-close").unbind('click');
			$("button.ui-dialog-titlebar-close").live('click',function(e){
				$(".ui-dialog").hide();
			});
		...
		*/
		...
		setInputObject:function(o,refs){
			...
			for(var prop in lbls){
				...
				html+="<div id=\""+lbls[prop].id+"\" class=\"labelItem\">"+name+"&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<button id=\"del_"+lbls[prop].id+"\" class=\"button shape delete formLink btn btn-info btn-xs pull-right\">Delete <span class=\"glyphicon glyphicon-remove\" aria-hidden=\"true\" title=\"Delete\"></span></button></div>";
		...
		*/
		sendLabels:function(){
			...
			$('li.cloneMe').each(function(i) {
				var n = i+1;
				var l=$('select#formLabel'+n).val();
				
				//$('select#formLabel'+n).change(function() {
				 var id = $(this).find("select#formLabel"+n+" option:selected").attr("id"); 
				//});
				
				//var id=$('select#formLabel'+n+' option').id();

				
				if($.inArray(l,lbls)<0){
					lbls[i] = $('select#formLabel'+n).val();
					ids[i]=id;
				}
			});
				/*console.log('ll');
			...
			for(var x in lbls){
				...
				if($.inArray(lbls[x],self.labelNames)<0){
					...
					el={id:ids[x],type:'labels',jsonName:'labels',name:lbls[x],obj:{id:ids[x],name:lbls[x]}};
					self._labels[ids[x]]=el;
				...
				html+="<div id=\""+el.id+"\" class=\"labelItem\">"+el.name+"&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<button id=\"del_"+el.id+"\" class=\"button shape delete formLink btn btn-info btn-xs pull-right\">Delete <span class=\"glyphicon glyphicon-remove\" aria-hidden=\"true\" title=\"Delete\"></span></button></div>";//  pull-right
	...
	var TILE_ENGINE=function(args){
		...
		self.serverStateUrl = (urls.state ? urls.state : "edit_imbrex_tile/isJSON");
		// 
			...
			switch(section){
					...
					if(area=='toolbar'){
						...
						if($("#azcontentarea > .az.inner."+styleName).length){
							if($("#azcontentarea > .az.inner."+styleName+" .toolbar").length){
								...
								var toolWrapper=''+html+'';
								$(toolWrapper).prepend("#azcontentarea > .az.inner."+styleName);
								$("#azcontentarea > .az.inner."+styleName).prepend(toolWrapper);
							}
						}
						if($("#azcontentarea > .az.inner."+styleName+" > .toolbar .panel-heading").length==0){
							// make a title for the plugin
							$("#azcontentarea > .az.inner."+styleName+" > .toolbar").prepend('<span class="menuitem pluginTitle">'+self.name+'</span>');
						}
					} else {
				...
				case 'topleft':
					...
					if(area=='toolbar'){
						...
						if($("#az_log > .az.inner."+styleName+" > .toolbar.panel-heading").length==0){
							// make a title for the plugin
	...
	*/
		...
		*/
		submitFileHandle:function(e){
			...
			$.ajax({
				// TODO: CHANGE THIS TO DYNAMIC SETTINGS
				url:'plugins/CoreData/importDataScript.php',
				//url:'imbrex/browse_imbrex_tile/importDataScript',
				cache:false,
	...
	TileToolBar=function(args){
		...
		*/
			...
			e.preventDefault();
			showLoad();
			var values = JSON.stringify(deepcopy(json));
			
			//console.log('save:');
			//console.log(JSON.stringify(deepcopy(json)));
						// handle the submit call to PHP
			$.ajax({
				// TODO: CHANGE THIS TO DYNAMIC SETTINGS
				url:'edit_imbrex_tile/exportDataScript',
				cache:false,
				data:({values}),
				type:'POST',
				dataType:'json',
