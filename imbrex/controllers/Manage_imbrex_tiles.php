 ...
 */
class Manage_imbrex_tiles extends CMS_CRUD_Controller {

    protected $URL_MAP = array();
    protected $TABLE_NAME = 'imbrex_text_body_lg';
    protected $COLUMN_NAMES = array('xml_id', 'file', 'pagenum', 'linebreaks', 'pagecontent', 'closed', 'shapes', 'selections');
    protected $PRIMARY_KEY = 'page_id';
    protected $UNSET_JQUERY = TRUE;
    protected $UNSET_READ = TRUE;
    protected $UNSET_ADD = TRUE;
    protected $UNSET_EDIT = FALSE;
    protected $UNSET_DELETE = TRUE;
    protected $UNSET_LIST = FALSE;
    protected $UNSET_BACK_TO_LIST = FALSE;
    protected $UNSET_PRINT = TRUE;
    protected $UNSET_EXPORT = TRUE;
    ...
    protected function make_crud(){
        ...
        //$crud->edit_fields('xml_id', 'pagenum', 'linebreaks','closed', 'pagecontent', 'file', '_updated_by', '_updated_at');
        $crud->edit_fields('closed','_updated_by', '_updated_at');
        ...
        //$crud->field_type('file', 'readonly');
        $crud->field_type('pagenum', 'readonly');
        $crud->field_type('pagecontent', 'readonly');
		...
		/*elseif($state=='list' OR $state=='ajax_list' OR $state=='success')
		{	
			$upload_path='';
			$crud->callback_column('file',array($this,'list_set_file_path'));
			$page_id=end($this->uri->segments);
			if(is_numeric($page_id)){
				$query=$this->get_row($this->t($this->TABLE_NAME), $page_id);
				$sel_row = $query->row_array(); 
				//$crud->where('`cms_imbrex_text_body_lg`.`xml_id` =', $sel_row['xml_id']);
			}
		}*/		
		...
		//$pk=$crud->getState();
		//var_dump($pk);
		$crud->add_action('EditTiles', '','','edit_button btn btn-default',array($this,'edit_tile'));

        $this->CRUD = $crud;
        return $crud;
    }

function edit_tile($primary_key , $row)
{
    return site_url($this->cms_module_path().'/edit_imbrex_tile').'?page_id='.$row->page_id;
}
    ...
    public function index(){
        ...
        $privileges = array(
            'allow_navigate_backend'    => $this->cms_allow_navigate($this->n('manage_imbrex_tiles')),
            ...
            'backend_url'               => site_url($module_path.'/manage_imbrex_tiles/index'),
        ...
        // show the view
        $this->view($this->cms_module_path().'/Manage_imbrex_tiles_view', $output,
            $this->n('manage_imbrex_tiles'), $config);
    }
	public function _callback_field_pagecontent($value,$row){
		$values = preg_split("/\\r\\n|\\r|\\n/", $value);
		//var_dump($values);
		$nvalue='<div id="logbar_list" class="az">';
		foreach($values as $k=>$v){
			$nvalue.="<div id=\"line".$k."\" class=\"line\">".$v."</div>";
			}
		$nvalue.="</div>";
		return $nvalue;
		}
    ...
    public function _after_insert_or_update($post_array, $primary_key){
        ...
        $this->_save_one_to_many(
            ...
            $data, // DATA
            array('id', 'type', '_scale', 'color', 'posinfo_x_cx', 'posinfo_y_cy', 'posinfo_width_rx', 'posinfo_height_ry', 'labels_lines'), // REAL DETAIL COLUMN NAMES
        ...
        $this->_save_one_to_many(
            ...
            $primary_key, // PARENT PRIMARY KEY VALUE
            $data, // DATA
            array('id', 'startparent', 'startoffset', 'startchild', 'endparent', 'endoffset', 'endchild', 'color'), // REAL DETAIL COLUMN NAMES
            array(), // SET DETAIL COLUMN NAMES
            $many_to_many_config_list=array()
        );
