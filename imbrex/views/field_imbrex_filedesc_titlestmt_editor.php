<?php
    ...
    $HTML = build_md_html_table('titlestmt_editor', 'Editor', array('Type', 'Name', 'Key'));
    // Generate global variable and event-binding
    $JS   = build_md_global_variable_script('titlestmt_editor', 'editor_id', $date_format, $result, $options);
    $JS  .= build_md_event_script('titlestmt_editor', '{{ module_site_url }}manage_imbrex_filedesc/index/insert', '{{ module_site_url }}manage_imbrex_filedesc/index/update');
    // Show HTML
    echo $HTML;
    // Show JS
...
<script type="text/javascript">
    ...
    function default_row_titlestmt_editor(){
        return {
            type : '',
            name : '',
            key : '',
    ...
    // Function to add row
    function add_table_row_titlestmt_editor(value){

        // Prepare some variables
        var input_prefix = 'md_field_titlestmt_editor_col';
        var row_index    = RECORD_INDEX_titlestmt_editor;
        ...
        html += build_single_select_option(field_value, OPTIONS_titlestmt_editor.type);
        html += '</select>';
        inputs.push(html);
