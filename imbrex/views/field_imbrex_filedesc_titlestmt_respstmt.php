<?php
    ...
    $HTML = build_md_html_table('titlestmt_respstmt', 'Responsibility (Other than author and editor)', array('Responsibility', 'Type', 'Name', 'Key'));
    // Generate global variable and event-binding
    $JS   = build_md_global_variable_script('titlestmt_respstmt', 'resp_id', $date_format, $result, $options);
    $JS  .= build_md_event_script('titlestmt_respstmt', '{{ module_site_url }}manage_imbrex_filedesc/index/insert', '{{ module_site_url }}manage_imbrex_filedesc/index/update');
    // Show HTML
    echo $HTML;
    // Show JS
...
<script type="text/javascript">
    ...
    function default_row_titlestmt_respstmt(){
        return {
            resp : '',
            type : '',
            name : '',
    ...
    // Function to add row
    function add_table_row_titlestmt_respstmt(value){

        // Prepare some variables
        var input_prefix = 'md_field_titlestmt_respstmt_col';
        var row_index    = RECORD_INDEX_titlestmt_respstmt;
        ...
        html += build_single_select_option(field_value, OPTIONS_titlestmt_respstmt.type);
        html += '</select>';
        inputs.push(html);
